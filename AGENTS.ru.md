# AGENTS.ru.md — Инструкция для AI-агентов по обновлению памяти
...
1. `system_prompt_block()` упоминает 3 бэкенда + SecureStore
2. `SEARCH_SCHEMA` описывает 3 бэкенда
3. `SECURE_GET_SCHEMA` предоставляет инструмент `hybrid_secure_get(key)`
4. `HybridMemoryProvider.__init__()` использует `LocalEmbedder` + `MemoryGraphBackend`
...
При пересборке убедись что `docker-compose.yml` содержит:
```yaml
environment:
...
def recency_boost(created_str: str) -> float:
    сегодня     → 1.00
    1-7 дней    → 1.00 → 0.60  (линейно)
    8-30 дней   → 0.60 → 0.30  (линейно)
    31-90 дней  → 0.30 → 0.05  (линейно)
    90+ дней    → 0.05
